...
def get_args():
    ...
    # Model arguments
    parser.add_argument('--yolo_weights', type=str, required=True, help="Path to best.pt")
...
def run_inference(dataloader, yolo_model, resnet_model, diameters, device):
    ...
    for batch_images, batch_targets in tqdm(dataloader):
        ...
        for i, detections in enumerate(batch_detections):
            ...
            img_np = (img_tensor.permute(1, 2, 0).numpy() * 255).astype(np.uint8) #(H, W, C)
            ...
            for det in detections:
                ...
                if linemod_id is None: continue
                ...
                results.append({
                    ...
                    't': t_pred.tolist() if isinstance(t_pred, np.ndarray) else t_pred
                })
...
# --- LOCAL IMPORTS ---
# Ensure LinemodDataset is the one we defined previously
from dataset import LinemodDataset, YOLO_TO_LINEMOD_ID 

# Replace these with your actual module imports
from my_yolo_module import load_yolo, get_yolo_batch_detections
from my_resnet_module import load_resnet, preprocess_patch
from my_geometry_module import pinhole_camera_translation, quaternion_to_matrix
...
def get_args():
    ...
    parser.add_argument('--split', type=str, default='val', choices=['train', 'val'], 
                        help="Which split to run inference on")
    parser.add_argument('--models_info', type=str, default='data/linemod/models/models_info.yml',
                        help="Path to LINEMOD models_info.yml for object diameters")
...
def run_inference(dataloader, yolo_model, resnet_model, diameters, device):
    ...
    # UPDATED LOOP: dataset returns (image, target_dict)
    # dataloader with collate_fn returns (list_of_images, list_of_dicts)
    for batch_images, batch_targets in tqdm(dataloader):
        
        # 1. Prepare Batch for YOLO
        # Stack list of tensors into (B, C, H, W) and move to device
        images_tensor = torch.stack(batch_images).to(device)
        
        # --- 1. YOLO Stage ---
        # Returns list of detections per image: [[x, y, w, h, class_id], ...]
        # (Assuming get_yolo_batch_detections accepts the stacked tensor)
        batch_detections = get_yolo_batch_detections(yolo_model, images_tensor)
        ...
        for i, detections in enumerate(batch_detections):
            ...
            # Extract Metadata from the new dictionary structure
            target = batch_targets[i]
            # Convert intrinsics to numpy if your geometry module expects numpy
            K = target['intrinsics'].numpy() 
            filename = target['image_id']
            ...
            for det in detections:
                # Assuming det is: x_c, y_c, w, h, cls_id
                ...
                # Ensure K is in the format your geometry module expects
                t_pred = pinhole_camera_translation(bbox, K, obj_diameter)
                
                # --- 4. Rotation (ResNet) ---
                # Preprocess patch (crop & resize)
                # Pass the numpy image and the bbox
                patch = preprocess_patch(img_np, bbox).to(device)
...
def main():
    ...
    dataset = LinemodDataset(
        ...
        split=args.split,
        img_ext='.jpg' # Change to .png if necessary
    )
